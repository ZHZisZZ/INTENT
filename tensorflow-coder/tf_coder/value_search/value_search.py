...
def _add_constants_and_inputs_and_print(
  ...
  # Print other input/outputprint('Example #{}'.format(i+1))
  for i in range(1, len(benchmark.examples)):
    print('Example #{}'.format(i+1))
    input_names_to_objects = _input_names_to_objects(benchmark.examples[i].inputs)
    for name, input_object in input_names_to_objects.items():
      input_value = value_module.InputValue(input_object, name)
      print("Input '{}':\n{!s}\n".format(name, input_value.value))
    print('Output:\n{!s}\n'.format(value_module.OutputValue(benchmark.examples[i].output).value))
...
def _check_application(expression: Text, 
                       example: benchmark_module.Example) -> Tuple[bool, bool, Text]:
  """Helper, returning (match, exception, eval_output)"""
  namespace = {'tf': tf}
  input_names_to_objects = _input_names_to_objects(example.inputs)
  for input_name, input_object in input_names_to_objects.items():
    ...
    namespace[input_name] = input_value.value

  try:
    new_output = eval(expression, namespace)
    new_output_value = value_module.OperationValue(new_output)
    # test object_to_string can be called with new_output_value
    repr(new_output_value)
  except Exception as e:
    return False, True, str(e)

  output_value = value_module.OutputValue(example.output)
  if output_value != new_output_value:
    return False, False, dataflow_utils.object_to_string(new_output_value.value)
    
  return True, False, dataflow_utils.object_to_string(output_value.value)
...
def _record_solutions(value: value_module.Value,
  ...
  for expression, used_input_names, reconstructed_value in reconstructions:
    ...
    if not all(_check_application(expression, example)[0] 
        for example in benchmark.examples):
      continue 

    solution_expression_set.add(expression)
    solutions.append(Solution(value=reconstructed_value, expression=expression,
...
def _find_solutions(
  ...
  solutions = asyn.solutions if asyn else []
...
def operation_multipliers_from_settings(settings: settings_module.Settings):
  operation_settings = settings.operations
  desired_multiplier = dict(map(
        lambda k: (k, operation_settings.desired_multiplier), operation_settings.desired_operations))
  undesired_multiplier = dict(map(
        lambda k: (k, operation_settings.undesired_multiplier), operation_settings.undesired_operations))
  multiplier = _combine_multipliers(desired_multiplier, undesired_multiplier)
  return multiplier
...
def get_reweighted_operations(
  ...
  multipliers = _combine_multipliers(
        multipliers, 
        operation_multipliers_from_settings(settings))
...
def run_value_search(
    ...
    tensor_model: Optional[tensor_features_model.Model] = None,
    tensor_config: Optional[Dict[Text, Any]] = None,
    asyn: asyn_utils.Asyn = None) -> ValueSearchResults:
  ...
  if solutions:
    ...
    if settings.max_solutions != 1:
      for i in range(1, len(solutions)): print(solutions[i].expression)
...
def run_value_search_from_example(
    inputs_list: List[Union[List[Any], Dict[Text, Any]]],
  ...
  benchmark = benchmark_module.Benchmark(
      ...
      examples = [benchmark_module.Example(inputs, output) 
          for inputs, output in zip(inputs_list, output_list)],
